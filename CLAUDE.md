...
- **Только факты проекта.** Перед утверждением о коде/файлах — Read/Grep. Про внешние сервисы — WebSearch.
- **Баги — чинить сразу** без «исправить?». Проверки и чтение файлов — сразу, без «посмотреть?».
- **Деплой — ТОЛЬКО `git push origin master`.** НИКОГДА `railway up` — создает конфликт двух инстансов.
...
- `reminder_cancel` поддерживает отмену по тексту (`text="панадол"`) и по индексу (`index=1`) — агент не угадывает номер
- Эмодзи разрешены в ответах: 1-3 на ответ, органично под контекст. Markdown (`**`, `*`, `_`) по-прежнему запрещён
...
## Стиль общения
Mythos — плотно, без воды, сразу к сути. Не пересказывай diff, не нумеруй очевидное. Факт + результат.

## Логи Railway
При дебаге багов — смотреть логи самостоятельно через `railway logs`.
```bash
railway logs -n 50              # последние 50 строк
railway logs --since 1h         # за последний час
railway logs -n 50 --filter "@level:error"  # только ошибки
```
CLI залогинен, проект tg-bot слинкован, сервис Worker привязан.

## Правила работы с терминалом

### Защита от зависших процессов
- Любая команда дольше 30 секунд без вывода — убить немедленно (Ctrl+C) и сообщить
- Никогда не ждать молча процесс который перестал отвечать
- После завершения зависшего процесса всегда сообщать: что зависло, сколько висело, последний вывод

### Потенциально долгие команды
Перед запуском `pip install`, `npm install`, сборок и dev-серверов — предупредить что команда может занять время.

### При ошибке или таймауте показывать
1. Команду которая упала
2. Код выхода или причину
3. Последние 10 строк вывода
